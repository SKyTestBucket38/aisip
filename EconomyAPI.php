        ...
        if ($p instanceof Player && $sender instanceof Player) {
            switch ($command->getName()) {
                case "money";
                    ...
                    $sender->sendMessage($this->getMessage("mymoney-mymoney", $sender->getName(), array($result, "%2", "%3", "%4")));
                break;
                case "setmoney";
                    ...
                    if(trim($player) === "" or trim($money) === ""){
                        $sender->sendMessage("§c* §fИспользование: §e/setmoney <игрок> <кол-во>");
                        break;
                    }
                    ...
                    if(!file_exists("Economy/".strtolower($p->getName()).".yml")){
                        ...
                        switch($result){
                            case -2: // CANCELLED
                                ...
                                break;
                            case -1: // NOT_FOUND
                                $output .= $this->getMessage("player-never-connected", $sender->getName(), array($player, "%2", "%3", "%4"));
                                break;
                            // INVALID is already checked
                            case 1: // SUCCESS
                                $output .= $this->getMessage("givemoney-gave-money", $sender->getName(), array($money, $player, "%3", "%4"));
                                file_put_contents("Economy/".strtolower($p->getName()).".yml", date("m.d.y"));
                                if($p instanceof Player){
                                    $p->sendMessage($this->getMessage("givemoney-money-given", $sender->getName(), array($money, "%2", "%3", "%4")));
                                }
                                break;
                        }
                ...
                case "pay";
                    ...
                    if(trim($player) === "" or trim($amount) === "" or !is_numeric($amount)){
                        $sender->sendMessage("§c* §fИспользование §e/pay (игрок) (количество)");
                        return true;
                    }
